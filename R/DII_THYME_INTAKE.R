#' DII_THYME_INTAKE: Calculate the Thyme Component within the DII
#' @description
#' To calculate the thyme component within the Dietary Inflammatory Index (DII) for NHANES participants (NHANES 2001-2020), it is not recommended to use NHANES 1999-2000 due to the absence of an FNDDS file for that survey period. Recipes used to calculate thyme intake are detailed in the file titled 'Recipes used to calculate the intakes of garlic, ginger, onion, pepper, thyme, and oregano.xlsx', which can be downloaded from the following link: \url{https://github.com/zxucmu/dietaryindexNDP/blob/41f72620337c693037f7244ec5b88821d953df9e/Recipes%20used%20to%20calculate%20the%20intakes%20of%20garlic%20ginger%20onion%20pepper%20thyme%20and%20oregano.xlsx}.
#'
...
#'
#' @return A dataframe with the variable name SEQN, and one or both of DR1TTHYME and DR2TTHYME. For DRXIFF_B, the variable name is DRXTTHYME.
#' @export
#'
#' @examples
#' # Example 1
#' DII_THYME_INTAKE(FLATTENED_FILEPATH = fnddsSRlinks_4.1_flattened,
...
#' # Example 2
#' DII_THYME_INTAKE(FLATTENED_FILEPATH = fnddsSRlinks_1314_flattened,
...
#' # Example 3
#' DII_THYME_INTAKE(FLATTENED_FILEPATH = fnddsIngred_1718_flattened,
...
#' # Example 4
#' DII_THYME_INTAKE(FLATTENED_FILEPATH = fnddsSRlinks_1.0_flattened,
...
DII_THYME_INTAKE <- function(FLATTENED_FILEPATH = NULL, DR1IFF_PATH = NULL, DR2IFF_PATH = NULL){
  ...
  message <- paste0(' The calculation of the thyme component within DII has been initiated. Please allow a moment for this process to complete')
  cat('\n')
  for (i in 1:3) {
  ...
  recipe <- c('02042')
  # SR/Ingredient code 02042: Spices, thyme, dried.
  # For more details about the SR/Ingredient codes used in calculating the thyme component for DII,
  ...
  if (!is.null(FLATTENED_FILEPATH) & !is.null(DR1IFF_PATH)) {
    ...
    cat('\n  Performing the calculation for the thyme component of DII based on the Individual Foods File from the first day.\n')

    if(("DR1ILINE" %in% colnames(DR1IFF))){
      dr1iff.thyme.SEQN <- setdiff(unique(DR1IFF$SEQN), unique(DR1IFF$SEQN[is.na(DR1IFF$DR1IGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr1iff.thyme.SEQN), style = 3)

      dr1tot.thyme <- sapply(dr1iff.thyme.SEQN, simplify = F, function(x){

        setTxtProgressBar(pb, which(x == dr1iff.thyme.SEQN))
        ...
        dplyr::rename(., 'DR1TTHYME' = 'Recipe')
      cat('\n')
    }
    if(("DRXILINE" %in% colnames(DR1IFF))){
      dr1iff.thyme.SEQN <- setdiff(unique(DR1IFF$SEQN), unique(DR1IFF$SEQN[is.na(DR1IFF$DRXIGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr1iff.thyme.SEQN), style = 3)
      dr1tot.thyme <- sapply(dr1iff.thyme.SEQN, simplify = F, function(x){
        setTxtProgressBar(pb, which(x == dr1iff.thyme.SEQN))
        DR1IFF.filter <- DR1IFF %>%
        ...
        dplyr::rename(., 'DRXTTHYME' = 'Recipe')
      cat('\n')
      if(!is.null(DR2IFF_PATH)){
        DR2IFF_PATH <- NULL
        cat('\n  Since there is only dietary intake data of one day included in NHANES 2001-2002, the thyme consumption is calculated based solely on the file pointed to by DR1IFF_PATH when both DR1IFF_PATH and DR2IFF_PATH lead to the same file.\n')
      }
    }
  }
  ...
  if (!is.null(FLATTENED_FILEPATH) & !is.null(DR2IFF_PATH)) {
    ...
    cat('\n  Performing the calculation for the thyme component of DII based on the Individual Foods File from the second day.\n')

    if(("DR2ILINE" %in% colnames(DR2IFF))){
      dr2iff.thyme.SEQN <- setdiff(unique(DR2IFF$SEQN), unique(DR2IFF$SEQN[is.na(DR2IFF$DR2IGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr2iff.thyme.SEQN), style = 3)
      dr2tot.thyme <- sapply(dr2iff.thyme.SEQN, simplify = F, function(x){
        setTxtProgressBar(pb, which(x == dr2iff.thyme.SEQN))
        ...
        dplyr::rename(., 'DR2TTHYME' = 'Recipe')
      cat('\n')
    }
    if(("DRXILINE" %in% colnames(DR2IFF))){
      dr2iff.thyme.SEQN <- setdiff(unique(DR2IFF$SEQN), unique(DR2IFF$SEQN[is.na(DR2IFF$DRXIGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr2iff.thyme.SEQN), style = 3)
      dr2tot.thyme <- sapply(dr2iff.thyme.SEQN, simplify = F, function(x){
        setTxtProgressBar(pb, which(x == dr2iff.thyme.SEQN))
        DR2IFF.filter <- DR2IFF %>%
  ...
  cat('\n  The calculation for the thyme component of DII has been done.\n')

  if(!is.null(FLATTENED_FILEPATH) & !is.null(DR1IFF_PATH)){
    if(!is.null(FLATTENED_FILEPATH) & !is.null(DR2IFF_PATH)){
      dr12tot.thyme <- dplyr::full_join(dr1tot.thyme, dr2tot.thyme, by = 'SEQN')
      return(dr12tot.thyme)
    }else{
      return(dr1tot.thyme)
    }
  }else{
