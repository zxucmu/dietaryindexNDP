#' DII_GARLIC_INTAKE: Calculate the Garlic Component within the DII
#' @description
#' To calculate the garlic component within the Dietary Inflammatory Index (DII) for NHANES participants (NHANES 2001-2020), it is not recommended to use NHANES 1999-2000 due to the absence of an FNDDS file for that survey period. Recipes used to calculate garlic intake are detailed in the file titled 'Recipes used to calculate the intakes of garlic, ginger, onion, pepper, thyme, and oregano.xlsx', which can be downloaded from the following link: \url{https://github.com/zxucmu/dietaryindexNDP/tree/Supplements/Example%20files%20for%20version%201.0.0}.
#'
...
#'
#' @return A dataframe with the variable name SEQN, and one or both of DR1TGARLIC and DR2TGARLIC. For DRXIFF_B, the variable name is DRXTGARLIC.
#' @export
#'
#' @examples
#' # Example 1
#' DII_GARLIC_INTAKE(FLATTENED_FILEPATH = fnddsSRlinks_4.1_flattened,
...
#' # Example 2
#' DII_GARLIC_INTAKE(FLATTENED_FILEPATH = fnddsSRlinks_1314_flattened,
...
#' # Example 3
#' DII_GARLIC_INTAKE(FLATTENED_FILEPATH = fnddsIngred_1718_flattened,
...
#' # Example 4
#' DII_GARLIC_INTAKE(FLATTENED_FILEPATH = fnddsSRlinks_1.0_flattened,
...
DII_GARLIC_INTAKE <- function(FLATTENED_FILEPATH = NULL, DR1IFF_PATH = NULL, DR2IFF_PATH = NULL){
  ...
  message <- paste0(' The calculation of the garlic component within DII has been initiated. Please allow a moment for this process to complete')
  cat('\n')
  for (i in 1:3) {
  ...
  recipe <- c('11215', '02020')
  # SR/Ingredient code 11215: Garlic, raw.
  # SR/Ingredient code 02020: Spices, garlic powder.
  # For more details about the SR/Ingredient codes used in calculating the garlic component for DII,
  ...
  if (!is.null(FLATTENED_FILEPATH) & !is.null(DR1IFF_PATH)) {
    ...
    cat('\n  Performing the calculation for the garlic component of DII based on the Individual Foods File from the first day.\n')

    if(("DR1ILINE" %in% colnames(DR1IFF))){
      dr1iff.garlic.SEQN <- setdiff(unique(DR1IFF$SEQN), unique(DR1IFF$SEQN[is.na(DR1IFF$DR1IGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr1iff.garlic.SEQN), style = 3)

      dr1tot.garlic <- sapply(dr1iff.garlic.SEQN, simplify = F, function(x){

        setTxtProgressBar(pb, which(x == dr1iff.garlic.SEQN))
        ...
        dplyr::rename(., 'DR1TGARLIC' = 'Recipe')
      cat('\n')
    }
    if(("DRXILINE" %in% colnames(DR1IFF))){
      dr1iff.garlic.SEQN <- setdiff(unique(DR1IFF$SEQN), unique(DR1IFF$SEQN[is.na(DR1IFF$DRXIGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr1iff.garlic.SEQN), style = 3)
      ...
      dr1tot.garlic <- sapply(dr1iff.garlic.SEQN, simplify = F, function(x){
        ...
        dplyr::rename(., 'DRXTGARLIC' = 'Recipe')
      cat('\n')
  ...
  if (!is.null(FLATTENED_FILEPATH) & !is.null(DR2IFF_PATH)) {
    ...
    cat('\n  Performing the calculation for the garlic component of DII based on the Individual Foods File from the second day.\n')

    if(("DR2ILINE" %in% colnames(DR2IFF))){
      dr2iff.garlic.SEQN <- setdiff(unique(DR2IFF$SEQN), unique(DR2IFF$SEQN[is.na(DR2IFF$DR2IGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr2iff.garlic.SEQN), style = 3)

      dr2tot.garlic <- sapply(dr2iff.garlic.SEQN, simplify = F, function(x){
        setTxtProgressBar(pb, which(x == dr2iff.garlic.SEQN))
        ...
        dplyr::rename(., 'DR2TGARLIC' = 'Recipe')
      cat('\n')
    }
    if(("DRXILINE" %in% colnames(DR2IFF))){
      dr2iff.garlic.SEQN <- setdiff(unique(DR2IFF$SEQN), unique(DR2IFF$SEQN[is.na(DR2IFF$DRXIGRMS)]))
      pb <- txtProgressBar(min = 0, max = length(dr2iff.garlic.SEQN), style = 3)
  ...
  cat('\n  The calculation for the garlic component of DII has been done.\n')

  if(!is.null(FLATTENED_FILEPATH) & !is.null(DR1IFF_PATH)){
    if(!is.null(FLATTENED_FILEPATH) & !is.null(DR2IFF_PATH)){
      dr12tot.garlic <- dplyr::full_join(dr1tot.garlic, dr2tot.garlic, by = 'SEQN')
      return(dr12tot.garlic)
    }else{
      return(dr1tot.garlic)
    }
  }else{
      return(dr2tot.garlic)
  }
}
